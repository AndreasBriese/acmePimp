...
char		*fontnames[2] =
{
	...
	"/mnt/font/Menlo-Regular/15a/font", /*me*/
	"/mnt/font/Menlo-Bold/14a/font" /*me*/
};
...
threadmain(int argc, char *argv[])
{
	...
	globalautoindent = TRUE; /*me  = FALSE; */
	oriniginalcolors = FALSE; /*me*/
	...
	case 'l':
		...
		break;
	case 'o':/*me*/
		oriniginalcolors = TRUE;
		fontnames[0] = "/mnt/font/Monaco/14a/font";
		break;
	case 'm':
		mtpt = ARGF();
		if(mtpt == nil)
	...
	default:
	Usage:
		fprint(2, "usage: acme -a(utoindent) -(o)riginalstyle -c ncol -f fontname -F fixedwidthfontname -l loadfile -W winsize\n");
...
iconinit(void)
{
	...
	if(tagcols[BACK] == nil) {
		if (oriniginalcolors) {
			...
			tagcols[HIGH] = allocimage(display, Rect(0,0,1,1), RGBA32, 1, 0xaa97ffFF);
			tagcols[BORD] = allocimage(display, Rect(0,0,1,1), RGBA32, 1, 0x7591a8FF);//0688f8a6833e
			tagcols[TEXT] = allocimage(display, Rect(0,0,1,1), RGBA32, 1, 0xaa97ffFF);
			tagcols[HTEXT] = allocimage(display, Rect(0,0,1,1), RGBA32, 1, 0x222222FF);
	
			/* Yellow */
			textcols[BACK] = allocimage(display, Rect(0,0,1,1), RGBA32, 1, 0x181512FF);
			textcols[HIGH] = allocimage(display, Rect(0,0,1,1), RGBA32, 1, 0xaa97ffFF);
			textcols[BORD] = allocimage(display, Rect(0,0,1,1), RGBA32, 1, 0x7591a8FF);
			textcols[TEXT] = allocimage(display, Rect(0,0,1,1), RGBA32, 1, 0xeed4b2FF);
	...
	tmp = allocimage(display, Rect(0,0,1,1), RGBA32, 1, 0x949762FF);
	draw(modbutton, r, tmp, nil, ZP);
	freeimage(tmp);

	r = button->r;
	/* colbutton = allocimage(display, r, screen->chan, 0, DPurpleblue); */
	colbutton = allocimage(display, r, RGBA32, 1, 0x9f7155FF);
